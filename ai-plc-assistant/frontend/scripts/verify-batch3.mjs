/**
 * Batch 3 验证截图：确认设计系统改造未破坏现有 UI。
 *
 * 截 4 个核心页面 x 2 尺寸 = 8 张，与 Batch 2 verify 对比。
 */
...
const OUT_DIR = resolve('../../docs/frontend/screenshots/batch3-verify')
const BATCH2_DIR = resolve('../../docs/frontend/screenshots/batch2-verify')
  ...
  for (const vp of VIEWPORTS) {
    ...
    for (const p of PAGES) {
      ...
      try {
        ...
        const batch2Path = resolve(BATCH2_DIR, filename)
        let batch2Size = 0
        try { batch2Size = statSync(batch2Path).size } catch {}
        const ratio = batch2Size > 0 ? ((newSize / batch2Size) * 100).toFixed(1) : 'N/A'
        const delta = newSize - batch2Size
        console.log(`[OK] ${filename}: batch3=${newSize} batch2=${batch2Size} ratio=${ratio}% delta=${delta > 0 ? '+' : ''}${delta}`)
        ok++
      } catch (err) {
