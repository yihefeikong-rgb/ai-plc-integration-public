...
# 导入被测模块（从 edge-gateway/src/change_detector.py 提取的共享函数）
_EDGE_SRC = Path(__file__).parent.parent / "edge-gateway" / "src"
...
class TestChangeDetection:
    ...
    def test_change_exceeds_delta(self, tag_config, prev_values):
        ...
        prev_values["register.0"] = 25.0
        # delta=5, 25→31 = +6 > 5
        assert has_significant_change("register.0", 31.0, prev_values, tag_config)

    def test_negative_delta_change(self, tag_config, prev_values):
        """负方向超过 delta 也应视为变化"""
        prev_values["register.0"] = 25.0
        # delta=5, 25→19 = -6 < -5
        assert has_significant_change("register.0", 19.0, prev_values, tag_config)

    def test_exact_delta_boundary_is_change(self, tag_config, prev_values):
        """变化正好等于 delta 应视为变化（>= delta）"""
        prev_values["register.0"] = 25.0
        # delta=5, 25→30 = 5
        assert has_significant_change("register.0", 30.0, prev_values, tag_config)

    def test_value_without_delta_tracks_any_change(self, tag_config, prev_values):
        """无 delta 的标签应检测任何不同值"""
        prev_values["coil.0"] = 0
        assert has_significant_change("coil.0", 1, prev_values, tag_config)

    def test_bool_coil_change(self, tag_config, prev_values):
        """Bool 类型值变化应检测到"""
        prev_values["coil.0"] = False
        assert has_significant_change("coil.0", True, prev_values, tag_config)

    def test_bool_coil_no_change(self, tag_config, prev_values):
        """Bool 类型值相同不应变化"""
        prev_values["coil.0"] = True
        assert not has_significant_change("coil.0", True, prev_values, tag_config)

    def test_none_value_not_change(self, tag_config, prev_values):
        """None 值不应触发变化"""
        assert not has_significant_change("register.0", None, prev_values, tag_config)
    ...
    def test_unknown_tag_in_config(self, tag_config, prev_values):
        ...
        assert has_significant_change("unknown.tag", 20, prev_values, tag_config)

    def test_value_change_then_stable(self, tag_config, prev_values):
        """变化后 stable 的 prev 更新应正常工作"""
        prev_values["register.0"] = 20.0
        assert has_significant_change("register.0", 100.0, prev_values, tag_config)
        prev_values["register.0"] = 100.0
        assert not has_significant_change("register.0", 100.0, prev_values, tag_config)
...
class TestBoundsCheck:
    ...
    def test_value_within_bounds(self, tag_config):
        """值在阈值范围内不应视为超限"""
        assert not is_out_of_bounds("register.0", 60.0, tag_config)

    def test_value_exceeds_max(self, tag_config):
        """值超过 max 应视为超限"""
        assert is_out_of_bounds("register.0", 121.0, tag_config)

    def test_value_below_min(self, tag_config):
        """值低于 min 应视为超限"""
        assert is_out_of_bounds("register.0", -1.0, tag_config)

    def test_value_at_min_equal_not_out(self, tag_config):
        """值等于 min 不应视为超限"""
        assert not is_out_of_bounds("register.0", 0.0, tag_config)

    def test_value_at_max_equal_not_out(self, tag_config):
        """值等于 max 不应视为超限"""
        assert not is_out_of_bounds("register.0", 120.0, tag_config)

    def test_tag_without_threshold_not_out(self, tag_config):
        """无阈值的标签不应视为超限"""
        assert not is_out_of_bounds("coil.0", 1, tag_config)

    def test_none_not_out_of_bounds(self, tag_config):
        """None 值不应触发阈值告警"""
        assert not is_out_of_bounds("register.0", None, tag_config)

    def test_independent_tag_thresholds(self, tag_config):
        """不同标签的阈值独立生效"""
        # Pressure: min=0, max=16
        assert is_out_of_bounds("register.2", 17.0, tag_config)    # 超 max
        assert is_out_of_bounds("register.2", -1.0, tag_config)    # 低于 min
        assert not is_out_of_bounds("register.2", 8.0, tag_config)  # 正常

    def test_unknown_tag_not_out(self, tag_config):
        """不在 tag_config 中的标签不应触发阈值告警"""
        assert not is_out_of_bounds("register.99", 999.0, tag_config)

    def test_edge_case_very_large_value(self, tag_config):
        """极大值应视为超限"""
        assert is_out_of_bounds("register.0", 1e10, tag_config)

    def test_int_vs_float_equivalence(self, tag_config):
        """int 和 float 的阈值比较应一致"""
        assert not is_out_of_bounds("register.0", 100, tag_config)  # int
        assert not is_out_of_bounds("register.0", 100.0, tag_config)  # float
        assert is_out_of_bounds("register.0", 121, tag_config)
        assert is_out_of_bounds("register.0", 121.0, tag_config)
