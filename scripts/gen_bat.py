"""Generate run_p3_complete.bat with proper CRLF line endings."""
import os

bat_path = os.path.join(os.path.dirname(__file__), '..', 'run_p3_complete.bat')

lines = []
def L(s):
    lines.append(s)

L('@echo off')
L('setlocal enabledelayedexpansion')
L('')
L('cd /d "%~dp0"')
L('')
L('set PYTHON=D:\\Python3\\python.exe')
L('set SCRIPTS=mcp-servers\\tia-mcp')
L('set GOLDEN="D:\\PLC cheng xu\\TIA PLC CHENG XU\\demo\\factory_io1_golden.zip"')
L('set STORAGE="D:\\PLC cheng xu\\TIA PLC CHENG XU\\demo\\plcsim_storage"')
L('set PLC_IP=10.0.0.1')
L('')
L('echo.')
L('echo ============================================')
L('echo  AI to PLC - P3 End-to-End Pipeline')
L('echo  Restore PLCSIM - Compile - Download - Backup')
L('echo ============================================')
L('echo.')
L('')
L('REM ---- Step 0: Start PLCSIM from golden backup ----')
L('echo [1/4] Restoring PLCSIM Advanced instance from golden backup...')
L('%PYTHON% %SCRIPTS%\\plcsim_api.py restore factoryio %GOLDEN% %STORAGE% %PLC_IP%')
L('if errorlevel 1 (')
L('    echo [FAIL] PLCSIM restore failed')
L('    pause')
L('    exit /b 1')
L(')')
L('echo [OK] PLCSIM running')
L('echo.')
L('')
L('REM ---- Step 1: Pre-check ----')
L('echo [2/4] Verifying PLCSIM instance...')
L('%PYTHON% %SCRIPTS%\\plcsim_api.py list')
L('if errorlevel 1 (')
L('    echo [FAIL] PLCSIM not available')
L('    pause')
L('    exit /b 1')
L(')')
L('echo [OK]')
L('echo.')
L('')
L('REM ---- Step 2: Compile + Download ----')
L('echo [3/4] Compile + Download to PLCSIM...')
L('echo      (TIA Portal GUI will auto-start if needed)')
L('echo.')
L('%PYTHON% %SCRIPTS%\\download_to_plcsim.py --compile-first')
L('if errorlevel 1 (')
L('    echo.')
L('    echo [FAIL] Download failed. Manual steps:')
L('    echo   1. Open TIA Portal GUI')
L('    echo   2. Right-click PLC_1 - Download to device - Software')
L('    echo   3. Then run: scripts\\archive_golden.py')
L('    pause')
L('    exit /b 1')
L(')')
L('echo [OK] Download complete')
L('echo.')
L('')
L('REM ---- Step 3: Update Golden Backup ----')
L('echo [4/4] Updating golden backup...')
L('%PYTHON% scripts\\archive_golden.py')
L('if errorlevel 1 (')
L('    echo [WARN] Golden backup update failed (non-fatal)')
L(')')
L('echo.')
L('')
L('echo ============================================')
L('echo  [DONE] P3 End-to-End Pipeline Complete')
L('echo  PLCSIM [OK] - Compile [OK] - Download [OK]')
L('echo ============================================')
L('pause')

# Write with CRLF
with open(bat_path, 'wb') as f:
    for line in lines:
        f.write(line.encode('ascii') + b'\r\n')

# Verify
with open(bat_path, 'rb') as f:
    data = f.read()

crlf_count = data.count(b'\r\n')
lf_count = data.count(b'\n') - crlf_count
ascii_only = all(b < 128 or b in (13, 10) for b in data)
print(f'Wrote {len(data)} bytes')
print(f'CRLF: {crlf_count}, Lone LF: {lf_count}, ASCII: {ascii_only}')

# Show content
for line in data.split(b'\r\n'):
    print(line.decode('ascii'))
