...
def step3_download():
    """步骤3: 下载到 PLCSIM（使用 TiaWorker C#）"""
    print(f"\n{BLUE}{'='*60}{RESET}")
    print(f"{BLUE}步骤3: 下载到 PLCSIM{RESET}")
    print(f"{BLUE}{'='*60}{RESET}\n")
    
    dl_script = PROJECT_ROOT / "mcp-servers" / "tia-mcp" / "download_to_plcsim.py"
    if not dl_script.exists():
        log(f"下载脚本不存在: {dl_script}", "error")
        return False
    
    log("通过 TiaWorker (C#) 下载到 PLCSIM...")
    try:
        r = subprocess.run(
            [sys.executable, str(dl_script), "--tiaworker"],
            capture_output=True, text=True, timeout=200,
            encoding='utf-8', errors='replace',
        )
        output = (r.stdout + r.stderr)
        if "TiaWorker 下载成功" in output:
            log("下载成功 ✓", "ok")
            return True
        else:
            log(f"下载未成功，请检查日志", "warn")
            log(f"输出摘要: {output[-300:]}", "info")
            return False
    except subprocess.TimeoutExpired:
        log("下载超时（200s）", "error")
        return False
    except Exception as e:
        log(f"下载异常: {str(e)[:200]}", "error")
        return False


def step4_start_plcsim():
    """步骤3: 启动 PLCSIM 仿真"""
    print(f"\n{BLUE}{'='*60}{RESET}")
...
def step5_connect_factory_io():
    ...
    try:
        # 生成 auto.cfg
        # Factory I/O 控制台命令格式的 auto.cfg
        # 文档: https://docs.factoryio.com/manual/console/
        fio_progdata = r"C:\ProgramData\Real Games\Factory IO\auto.cfg"
        fio_home = os.path.join(os.path.expanduser('~'), 'Documents', 'Factory IO', 'auto.cfg')
        
        config = """# Factory I/O automatic config — generated by auto_full_pipeline.py
...
"""
        ...
        for cfg_path in [fio_progdata, fio_home]:
            os.makedirs(os.path.dirname(cfg_path), exist_ok=True)
            with open(cfg_path, 'w') as f:
                f.write(config)
            log(f"auto.cfg: {cfg_path}", "ok")
...
def main():
    ...
    parser.add_argument('--download', action='store_true', help='仅下载到 PLCSIM')
    parser.add_argument('--simulate', action='store_true', help='仅启动仿真')
    ...
    if args.all or args.download:
        results['download'] = step3_download()
    
    if args.all or args.simulate:
        results['simulate'] = step4_start_plcsim()
    
    if args.all or args.fio:
        results['fio'] = step5_connect_factory_io()
    ...
    log(f"  步骤1: 环境检查")
    log(f"  步骤2: 编译项目")
    log(f"  步骤3: 下载到 PLCSIM (TiaWorker)")
    log(f"  步骤4: 启动 PLCSIM 仿真")
    log(f"  步骤5: 连接 Factory I/O")
    print()
