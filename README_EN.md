...
| PLC Gateway | FastMCP exposes only nine read-only endpoints; shadow routing normalizes real `ToolResult` values and TiaWorker verifies the actual project path before every project read. Evidence remains offline only. | This does not prove real TIA/TiaCommander calls, dynamic shadow equivalence, write-chain integration, or TiaCommander validation. |
